...
# adapted for eXTP
from CustomInstrument_eXTP import SFA
from CustomPrior import CustomPrior
from parameter_values import parameter_values


# copied from NICER analysis
...
class analysis(object):
    ...
    def set_prior(self):
        self.prior = CustomPrior(self.scenario, self.bkg, fix_mass=self.fix_mass)
    ...
    def __call__(self):
        ...
        if self.run_type == 'plot':
            ...
            # Plot spectra
            star = self.signal.signals[0] * self.exposure_time / 30  # Normalize for computation phase bin width and multiply by exposure time to get expected counts
            star_spec = np.sum(star, axis=1)
            if 'disk' in self.bkg:
                disk = self.signal.signals[1] * self.exposure_time / 30
                disk_spec = np.sum(disk, axis=1)
            energies = get_mids_from_edges(self.instrument.channel_edges)
            channels = self.instrument.channels
            fig, ax = plt.subplots()
            ax.plot(channels, star_spec, label='star')
            ax.plot(channels, disk_spec, label='disk')
...
if __name__ == '__main__':
    Analysis = analysis('snellius', 'sample', 'disk', sampler='multi', scenario='small_r')
    Analysis()
