...
class parameter_values(object):
    ...
    def bounds(self):
        bounds = {'radius':(3.0 * gravradius(1.0), 16.0),
              ...
              'cos_inclination':(np.cos((77.4+8.5)*np.pi/180), np.cos((77.4-11.5)*np.pi/180)), #restricted via polarization info
              'phase_shift':(-0.25, 0.75),
              'super_colatitude':((12-3.6)*np.pi/180, (12+3.6)*np.pi/180), #restricted due to polarization info
