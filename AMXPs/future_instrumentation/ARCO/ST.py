...
class analysis(object):
    ...
    def __call__(self):
        ...
        profile1 = CustomAxes.plot_2D_counts(axes[0], self.data.counts, get_mids_from_edges(self.data.phases), get_mids_from_edges(self.instrument.channel_edges))
        profile2 = CustomAxes.plot_2D_counts(axes[1], self.signal.expected_counts, get_mids_from_edges(self.data.phases), get_mids_from_edges(self.instrument.channel_edges))
        profile3 = CustomAxes.plot_2D_counts(axes[2], self.signal.expected_counts-self.data.counts, get_mids_from_edges(self.data.phases), get_mids_from_edges(self.instrument.channel_edges))
        fig.colorbar(profile1, ax=axes[0])
        fig.colorbar(profile2, ax=axes[1])
        fig.colorbar(profile3, ax=axes[2])  
        axes[0].set_title('data')
        axes[1].set_title('model')
        axes[2].set_title('model-data')
        fig.tight_layout()
