...
class parameter_values(object):
    ...
    def bounds(self):
        ...
        bounds = {'radius':(3.0 * gravradius(1.0), 16.0),
              ...
              'cos_inclination':(np.cos((i_high)*np.pi/180), np.cos((i_low)*np.pi/180)), #restricted via polarization info
              'phase_shift':(-0.25, 0.75),
              'super_colatitude':((theta_low)*np.pi/180, (theta_high)*np.pi/180), #restricted due to polarization info
              ...
              'column_density': (0., 3.),
              'compactness': (0., 10.),
              'inclination_deg': (i_low, i_high),
              'colatitude_deg': (theta_low, theta_high),
              'radius_deg': (0.001, 90)              
              }
