...
from hot cimport (init_hot, eval_hot, eval_hot_norm, free_hot, eval_hot_faster) #multiply_pointers, multiply_vectors)
...
def interpolate(size_t N_T,
    ...
    clock_gettime(CLOCK_REALTIME, &te)
    t_elapsed = (te.tv_nsec - ts.tv_nsec)
    printf("new eval_hot() takes %ld ns\n",t_elapsed) 
